...
var $w = window.parent;
var llist;  //动态缓存
var page=1; //页数，页面加载就是第一页
var i=0;
$(document).ready(function() {
	checkyh();
	if (null != yh && "" != yh) {
		showDt(null,yh.uno,page);
	}
	...
	$(parent.window).scroll(function() {
		...
		$('#top').css({
			top : $(parent.window).scrollTop() + 593
		});
	});
});
...
function myTie(obj) {
	showDt(yh.uname,yh.uno);
}
//点击评论加载
function tjpll(obj, dtbh) {
	var z = obj;
	var para = {
		uname : yh.uname,
		dtno : dtbh,
	...
	$.post("/DietManagementSystem/PlbController/add.do", para, function(tjzt) {
		$(z).prev().val("");
		$.post("/DietManagementSystem/PlbController/select.do", {
			dtno : dtbh
		}, function(pllist) {
			$(z).parent().parent().prev().children("span").eq(1).html(pllist.length);
			var $plitem = $(z).parent().prev();
			if (pllist.length > 0) {
				var llist;
				$plitem.empty();
				for (var i = 0; i < pllist.length; i++) {
					...
					$plitem.append($("<div>").addClass("pl_item").append(
							...
							$("<span>").addClass("name_pl").append($("<div>").addClass("pl_name").html(llist.uname)).append($("<div>").addClass("pl_nr").html(llist.contain)).append(
									$("<div>").addClass("pl_date").html(llist.pdate))));
				}
...
function showDt(uname,uno,page) {
	$.post("/DietManagementSystem/DtController/FyList.do",{uname:uname,uno:uno,page:page},function(dtlist) {
		llist=dtlist;
		if(page==1){	//第一页时查询好自动拼接好
			pjdt(); //拼接
		}
