...
var i=0;
$(function(){
	$("#tie").parent().css("background-color","red");
	checkyh();
	if (null != yh && "" != yh) {
...
function showDt(uname,uno) {
	$.post("/DietManagementSystem/DtController/dtList.do",{uname:uname,uno:uno},function(dtlist) {
	...
	for(i;i<(once*fy+1);i++){
		...
		$("#item").append(
				$("<div>").addClass("item").attr("id", list.dtno).append(
						...
						$("<div>").addClass("item_content").html(list.content)).append($("<div>").addClass("del")).append(
